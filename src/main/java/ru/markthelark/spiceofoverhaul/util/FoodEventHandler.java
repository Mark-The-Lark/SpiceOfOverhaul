        ...
        if (Config.addGuiText && event.getOverlay() == GuiOverlayManager.findOverlay(PLAYER_HEALTH_ELEMENT))
        {
            ...
            if (!player.isDeadOrDying() && !player.isCreative()) {
                ...
                if (healthPercent <= 0.15F) {
                    GuiComponent.drawString(event.getPoseStack(), gui.getFont(), "dying", right-91-"dying".length()*6+1, top, -1);
                } else if (healthPercent <= 0.3F) {
                    GuiComponent.drawString(event.getPoseStack(), gui.getFont(), "injured", right-91-"injured".length()*6+1, top, -1);
                } else if (healthPercent < 0.5F) {
                    GuiComponent.drawString(event.getPoseStack(), gui.getFont(), "hurt", right-91-"hurt".length()*6+1, top, -1);
                }
                if (player.getFoodData().getFoodLevel() <= 6) {
                    GuiComponent.drawString(event.getPoseStack(), gui.getFont(), "starving", right+93, top, -1);
                } else if (player.getFoodData().getFoodLevel() <= 10) {
                    GuiComponent.drawString(event.getPoseStack(), gui.getFont(), "hungry", right+93, top, -1);
                } else if (player.getFoodData().getFoodLevel() <= 14) {
                    GuiComponent.drawString(event.getPoseStack(), gui.getFont(), "peckish", right+93, top, -1);
                }
            }
        }
    }
