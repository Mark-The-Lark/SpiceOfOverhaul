...
import ru.markthelark.spiceofoverhaul.mixin.FoodHelperMixin;
...
//        if (effect != null && player.hasEffect(effect)){return;}
    ...
    {
       if (Config.modifyFoodEatingSpeed && event.getItem().getFoodProperties(event.getEntity()) != null) {
            int hunger = event.getItem().getFoodProperties(event.getEntity()).nutrition();
